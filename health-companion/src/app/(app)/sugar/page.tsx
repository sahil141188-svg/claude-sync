...
import type { SugarReading, Profile } from '@/lib/types';
...
export default async function SugarPage({
  ...
  const [{ data: readings }, userRes] = await Promise.all([
    supabase.from('sugar_readings').select('*').gte('measured_at', since).order('measured_at'),
  ...
  return (
    <div className="space-y-4 animate-fade-in-up">
      ...
      {profile?.role === 'caregiver' && <ReadingForm />}
